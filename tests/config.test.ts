...
describe("legacy PySide configuration migration", () => {
  it("preserves identity, ports, launcher and monitoring settings", () => {
    ...
    expect(migrated).toMatchObject({ name: "Legacy", clientFullscreen: true, localWsPort: 25585, launcher: { path: "Zombears.exe", onSession: true }, monitoring: { processes: ["Zombears.exe"], cpuThreshold: 80 } });
  });
});
